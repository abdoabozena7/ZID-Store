...
function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (window.__zidExploreResize) {
        window.__zidExploreResize = null;
      }
      if (sketch) {
        sketch.stop();
      }
  ...
  const loadingText = 'استني اللبس بيحمل ';
...
const exploreRoot = document.getElementById('exploreReactRoot');
if (exploreRoot) {
